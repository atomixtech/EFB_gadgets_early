...
void EFBSerial::begin(long baudrate, const char* name, byte version, byte uid[6])
{
  _devName = name;
  _devVersion = version;
  _devUID[0] = uid[0];
...
{
  char bufOut[128];
  sprintf(bufOut, "{DD#%s#%u#%02x-%02x-%02x-%02x-%02x-%02x}", 
     _devName, _devVersion, 
     _devUID[0], _devUID[1], _devUID[2], _devUID[3], _devUID[4], _devUID[5]);
